package com.spotify.web.methods.excel;
...
public class CreateExcelMethods {
    ...
    public XSSFCellStyle createStyleDate(XSSFWorkbook workbook, String dateFormat){

        XSSFColor color2 = getColor(workbook,255, 255, 255, 255); // white
        CreationHelper createHelper = workbook.getCreationHelper();
        XSSFCellStyle styleDate = workbook.createCellStyle();
        styleDate.setDataFormat(createHelper.createDataFormat().getFormat(dateFormat));
        styleDate = workbook.createCellStyle();
        styleDate.setFillForegroundColor(color2);
        styleDate.setFont(getFont(workbook,12,false,false));
        styleDate = getStyle(styleDate,"CENTER","CENTER");
        return styleDate;
    }
    ...
    public void setCell(XSSFRow row, XSSFCellStyle style, int columnNumber, String cellType, String cellValue, String formula){
        ...
        if (cellType.equals("DATE")){
            ...
            cell.setCellValue(org.joda.time.LocalDateTime.parse(cellValue, DateTimeFormat.forPattern(formula)).toDate());
